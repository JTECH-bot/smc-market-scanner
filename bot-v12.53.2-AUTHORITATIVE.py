...
SCANNER_WORKFLOW = os.environ.get(
    "SCANNER_WORKFLOW",
    "LSE-Smart-Signal-Scanner-v12.53.2-AUTHORITATIVE-FINAL.yml",
).strip()
...
def telegram(method, data=None):
    response = requests.post(f"{API}/{method}", json=data or {}, timeout=35)
...
def trigger_scanner():
    ...
    headers = {
        ...
        "User-Agent": "LSE-Smart-Telegram-Bot-v12.53.2",
    }
    payload = {
        "ref": GITHUB_REF,
        "inputs": {
            ...
            "runtime_minutes": "8",
            "scan_mode": "ONCE",
            "until_position_max_minutes": "55",
            "fast_candidate_limit": "28",
            "fast_min_score": "50",
            "lse_request_min_interval": "1.20",
            "lse_429_cooldown": "12",
            "run_tca": "false",
            "tca_start": "",
            "tca_end": "",
            "deep_history_start": "2021-01-01",
            "deep_history_end": "",
        },
    }
...
def help_text():
    return (
        "📡 LSE SMART SIGNAL SCANNER v12.53.2\n\n"
        "Commands:\n"
        "/scan — start a fresh authoritative scanner run\n"
        "/start — show this help\n\n"
        "Scanner includes major/cross FX strength ranking, structural TP ≥300 pips, ≥2R, "
        "2-of-9 strategy confirmation, LSE bid/ask execution validation, controlled 3-minute entry window, "
        "pair replacement and entry expiry.\n\n"
        "Automatic trading: DISABLED."
    )


def main():
    print("LSE Telegram Bot v12.53.2 starting...")
            ...
            for update in result.get("result", []):
                ...
                if text == "/scan":
                    if not authorized(chat_id):
                        send_message(chat_id, "⛔ This chat is not authorized to trigger the scanner.")
                        continue
                    try:
                        trigger_scanner()
                        send_message(
                            ...
                            "GitHub Actions has started LSE Smart Signal Scanner v12.53.2.\n\n"
                            "The authoritative validated result will be sent here automatically.\n"
                            "Automatic trading: DISABLED.",
                        )
                ...
                if text:
                    send_message(chat_id, "Use /scan to start a fresh scanner run.\n\n" + help_text())
